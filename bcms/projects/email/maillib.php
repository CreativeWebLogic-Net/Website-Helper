...
function Attach( $filename, $filetype = "", $disposition = "inline",$superget=false,$change="")
{
...
function _build_attachement()
{
	...
	for( $i=0; $i < count( $this->aattach); $i++ ) {
		...
		if($this->asget[$i]){
			$msgText=join("",file($this->aattach[$i]));
			$msgText=eregi_replace("Insert Text Here",$this->achange[$i],$msgText);
			$ata[$k++] = chunk_split(base64_encode($msgText));
		}else{

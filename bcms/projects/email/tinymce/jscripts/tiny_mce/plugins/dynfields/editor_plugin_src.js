...
tinyMCE.importPluginLanguagePack('dynfields', 'en,he,no'); // <- Add a comma separated list of all supported languages
 ...
 */
function TinyMCE_dynfields_getInfo() {
	return {
		longname : 'dynfields plugin',
		author : 'daniel ruul',
		authorurl : 'http://www.iwebbiz.com.au',
		infourl : 'http://www.iwebbiz.com.au/',
		version : "1.0"
	};
};
 ...
 */
function TinyMCE_dynfields_initInstance(inst) {
 ...
 */
function TinyMCE_dynfields_getControlHTML(control_name) {
	switch (control_name) {
		case "dynfields":
			return '<a href="javascript:tinyMCE.execInstanceCommand(\'{$editor_id}\',\'mceOpenPopup\', true);" target="_self" onmousedown="return false;"><img id="{$editor_id}_dynfields" src="{$pluginurl}/images/template.gif" title="{$lang_dynfields_desc}" width="20" height="20" class="mceButtonNormal" onmouseover="tinyMCE.switchClass(this,\'mceButtonOver\');" onmouseout="tinyMCE.restoreClass(this);" onmousedown="tinyMCE.restoreAndSwitchClass(this,\'mceButtonDown\');" /></a>';
	}
 ...
 */
function TinyMCE_dynfields_execCommand(editor_id, element, command, user_interface, value) {
	// Handle commands
	switch (command) {
		...
		case "mceOpenPopup":
			...
			//if (user_interface) {
				...
				dynfields['file'] = '../../plugins/dynfields/popup.htm'; // Relative to theme
				dynfields['width'] = 300;
				dynfields['height'] = 200;

				tinyMCE.openWindow(dynfields, {editor_id : editor_id, some_custom_arg : "somecustomdata"});
 ...
 */
function TinyMCE_dynfields_handleNodeChange(editor_id, node, undo_index, undo_levels, visual_aid, any_selection) {
	// Deselect template button
	tinyMCE.switchClassSticky(editor_id + '_dynfields', 'mceButtonNormal');

	// Select template button if parent node is a strong or b
	if (node.parentNode.nodeName == "STRONG" || node.parentNode.nodeName == "B")
		tinyMCE.switchClassSticky(editor_id + '_dynfields', 'mceButtonSelected');
 ...
 */
function TinyMCE_dynfields_cleanup(type, content) {
	switch (type) {

<?php
    ...
    class clsContent  extends clsFormCreator{
        ...
        function Content_Init(){
            ...
            $current_page="/";
            $get_variables=array();
            foreach($this->content_data["uri_split_array"] as $key=>$val){
                $pos = strpos($val, '=');
                if($pos){
                    $parts = explode('=', $val);
            ...
            $this->content_data['get_variables']=$get_variables;
            //print_r($this->content_data['get_variables']);
            /*
            ...
            */
            if(!isset($this->content_data["TOTALPAGENAME"])){
                $this->content_data['RESET']=true;
            ...
            if(isset($this->domain_data["db"]["SEOFriendlyLT"])){
                if($this->domain_data["db"]["SEOFriendlyLT"]==13){
                ...
                }elseif($this->domain_data["db"]["SEOFriendlyLT"]==12){
                    print_r($this->content_data);
            ...
            if($num_rows>0){
                ...
                //print_r($_SESSION);
                if($this->content_data['db']['ExposureLT']==37){
            ...
            while(($csearch)&&($max_count<10)){
                ...
                if($num_rows>0){
                    ...
                    $this->content_data['db']=$this->r->Fetch_Assoc($rslt);	
                    print_r($this->content_data);
                    if(!isset($_SESSION['membersID'])&&($this->content_data['db']['ExposureLT']==37)){
                        //echo"Member Page";
                        exit("Member Page");
                        //header("Location: /");
                    }
                ...
                if($this->r->NumRows($rslt)==0){// cant find page so load homepage for language/site
                    ...
                    if($num_rows>0){
                        ...
                        if($this->content_data["original_uri"]=="/"){
                            // on homepage
                            $this->content_data['db']=$this->r->Execute_Database_Query("Retrieve","content_pages",
                            array("HomePage_item-groups-Boolean-ModalID"=>12,"languagesID"=>$_SESSION['LanguagesID'],"domainsID"=>$this->domain_data["db"]['id']));
                            ...
                            // on homepage
                            $this->content_data['db']=$this->r->Execute_Database_Query("Retrieve","content_pages",
                            array("URI"=>$this->content_data['PAGENAME'],"languagesID"=>$_SESSION['LanguagesID'],"domainsID"=>0));
            ...
            //print_r($this->content_data);
            if(isset($this->content_data['db']['module_viewsid'])){
                $content_id=$this->content_data['db']['module_viewsid'];
            }else{
                $content_id=0;
            }
        ...
        function Display(){
            ...
            //print_r($this->module_data);
            if(isset($this->module_data["db"]['Method'])){
        ...
        function Content_Pages_Add_Table(){
            return $this->output;
        }

        function Content_Pages_Edit_Table(){
            return $this->output;
        }

        function Content_Pages_List_Table(){

            $this->output=$this->Create_Content_Pages_List_Table();
            return $this->output;
        }

        function Pre_Modify_Page(){

        }

        function Pre_Add_Page(){
            
        }

        function Pre_Edit_Page(){
